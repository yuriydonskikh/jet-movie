...
namespace JetMovie.Data
{
    ...
    {
        public async Task<List<MovieViewModel>> GetFilmworldMovies(MovieRequest request)
        {
            var movies = await Task.Run(() => {
                return _cache.Use(RequestKey(request), () =>
                    FilmworldMovies.Include(i => i.MovieInfo).ThenInclude(i => i.Genres).ThenInclude(i => i.Genre)
        ...
        public async Task<MovieViewModel> GetFilmworldMovie(string id)
        {
            var result = await FilmworldMovies.Include(i => i.MovieInfo).ThenInclude(i => i.Genres).ThenInclude(i => i.Genre).FirstOrDefaultAsync(i => i.MovieInfo.Id == id);
